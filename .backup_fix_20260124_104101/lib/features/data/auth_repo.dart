...
class AuthRepo {
  ...
  Future<UserModel?> login({
    ...
    if (user.id.isEmpty || user.token == null || user.token!.isEmpty) {
      throw ApiError(message: 'Login succeeded but response was missing user/token.');
    }
  ...
  Future<UserModel?> register({
    ...
    final user = UserModel.fromAuthResponse(res);
    if (user.id.isEmpty || user.token == null || user.token!.isEmpty) {
      throw ApiError(message: 'Register succeeded but response was missing user/token.');
    }
