...
BACKUP_DIR=".bak_explore_upgrade_$TS"
...
// ✅ If this import fails due to constructor args mismatch,
// run the command under "IF IT DOESN'T COMPILE" at the bottom of my message.
...
class _ExploreScreenState extends State<ExploreScreen> {
  ...
  void _openListing(_DemoListing l) {
    // ✅ Best effort: open your existing ListingDetailScreen.
    // If your constructor needs args, see the bottom instructions.
      ...
      child: SafeArea(
        ...
        child: CustomScrollView(
          ...
          slivers: [
            ...
            // ✅ Buttons: Buy / Agents / Rent + Land / Commercial + Saved / Viewings / Alerts
            SliverToBoxAdapter(
              child: Padding(
                      ...
                      children: [
                        Expanded(
                          child: _SmallPill(
                            ...
                            text: 'Saved',
                            onTap: () => _toast('Saved (use bottom tab)'),
                          ),
                        ),
                        const SizedBox(width: 10),
                        Expanded(
                          child: _SmallPill(
                            ...
                            text: 'Viewings',
                            onTap: () => _toast('Viewings (use bottom tab)'),
                          ),
                        ),
                        const SizedBox(width: 10),
                        Expanded(
                          child: _SmallPill(
                            ...
                            text: 'Alerts',
                            onTap: () => _toast('Alerts (use bottom tab)'),
                          ),
                        ),
                      ],
                    ),
            ...
            // ✅ Featured carousel (left/right) with 15
            SliverToBoxAdapter(
              child: SizedBox(
                height: 286,
            ...
            // ✅ EXACT requirement:
            // - Latest listings should scroll left
            // - Max 3 lines/rows
            // - Every row scrollable
            SliverToBoxAdapter(
              child: Padding(
                padding: const EdgeInsets.fromLTRB(14, 0, 14, 0),
                child: Column(
            ...
            // Bottom padding for frosted nav overlay
...
class _ActionPill extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final bg = filled ? color.withValues(alpha: 0.92) : Colors.white.withValues(alpha: 0.62);
...
class _Dots extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // keep it neat: show max 7 dots centered around active
    final maxDots = 7;
...
echo
echo "✅ Explore updated (featured + latest horizontal, 15 sponsored, buttons restored)."
