...
BACKUP_DIR=".bak_fix_remaining_6_$TS"
mkdir -p "$BACKUP_DIR"
...
if ten.exists():
    ...
    def is_if_header(line: str) -> re.Match | None:
        # matches: if (something)   OR if (something) // comment
        return re.match(r'^(\s*)if\s*\((.*)\)\s*(//.*)?\s*$', line)

    def has_block(line: str) -> bool:
        # if (...) { already, or if (...) { on same line
    ...
    def net_balance(chunk: str) -> tuple[int,int]:
        # simple bracket tracking to know when a call/map/list likely ends
        # (good enough when combined with semicolon check)
    ...
    while i < len(lines):
        ...
        while k < len(lines):
            ...
            # statement end = semicolon line AND likely balanced
...
#    - remove ALL @override (with whitespace)
#    - add @override ONLY for the "user" getter/member
# --------------------------------------------
rs = Path("lib/features/auth/ui/register/register_state.dart")
if rs.exists():
    s0 = read(rs)

    # remove any @override line even with spaces/tabs
    s1 = re.sub(r'^\s*@override\s*$\n?', '', s0, flags=re.M)

    # add @override immediately above the first "user" getter/member line
    lines = s1.splitlines(True)
    out = []
    inserted = False

    # match getter:  UserModel? get user => ...
    # or field:     final UserModel? user;
    patt = re.compile(r'^\s*(?:final|late)?\s*.*\bget\s+user\b|^\s*(?:final|late)?\s*.*\buser\s*[,;=]', re.I)
...
# --------------------------------------------
# C) OPTIONAL: remove "value is deprecated" infos by enabling initialValue
#    for your custom AppFormField (if present),
#    then switching those two screens back to initialValue: ...
# --------------------------------------------
form_field = Path("lib/shared/widgets/form_field.dart")
if form_field.exists():
    s0 = read(form_field)

    # Only patch if it looks like a custom AppFormField
    if "class AppFormField" in s0 and "initialValue" not in s0:
        s = s0

        # 1) Add initialValue param in constructor (best-effort)
        # Find constructor parameter list that contains "this.value"
        s = re.sub(
        ...
        # 2) Add field declaration near "final ... value;"
        s = re.sub(
            r'(\n\s*final\s+[^;\n]*\s+value\s*;\s*\n)',
        ...
        # 3) If it uses TextFormField(initialValue: ...), prefer initialValue ?? value
        s = re.sub(
        ...
        # 4) If it uses controller.text assignment from value, also respect initialValue
        s = re.sub(
...
# Switch those two screens from value: -> initialValue: (ONLY if they still contain value:)
targets = [
...
for p in targets:
    if not p.exists():
        continue
    s0 = read(p)
    # only change value: to initialValue: when it appears inside AppFormField(...)
    s1 = re.sub(r'(AppFormField\s*\([^;]*?)\bvalue\s*:', r'\1initialValue:', s0, flags=re.S)
    if s1 != s0:
        write(p, s1)
