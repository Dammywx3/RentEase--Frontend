...
BACKUP_DIR=".bak_explore_mock_$TS"
mkdir -p "$BACKUP_DIR"
...
mkdir -p lib/core/ui/scaffold
mkdir -p lib/core/ui/nav
mkdir -p lib/features/tenant/shell
mkdir -p lib/features/tenant/explore

echo "🛠️ Writing: lib/core/ui/scaffold/app_scaffold.dart"
cat > lib/core/ui/scaffold/app_scaffold.dart <<'DART'
            ...
            sliver: SliverGrid(
              delegate: SliverChildBuilderDelegate(
                (context, i) => _ListingMiniCard(
                  ...
                  meta: i.isEven ? '4 bds | 3 ba | 2,676 sqft' : '3 bds | 2 ba | 1,900 sqft',
            ...
            AspectRatio(
              ...
              child: Image.network(
                ...
                errorBuilder: (_, __, ___) => Container(
                  color: cs.surface,
                  child: Center(
                    child: Icon(Icons.image_outlined, size: 44, color: cs.outline),
                  ),
                ),
              ),
            ),
...
class _QuickActions extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    Widget pill({
      ...
      final bg = filled ? cs.primary.withValues(alpha: 0.18) : cs.surface.withValues(alpha: 0.80);
                ...
                children: [
                  Positioned.fill(
                    child: Image.network(
                      ...
                      errorBuilder: (_, __, ___) => Container(
                        color: cs.surface,
                        child: Center(child: Icon(Icons.image_outlined, color: cs.outline)),
                      ),
                    ),
                  ),
                  Positioned(
                    ...
                    child: Container(
                      ...
                      child: Icon(Icons.favorite_border_rounded, size: 18, color: cs.onSurface),
                    ),
                  ),
                ],
              ),
