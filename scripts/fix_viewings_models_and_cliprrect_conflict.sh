...
BACKUP_DIR=".bak_fix_viewings_$TS"
mkdir -p "$BACKUP_DIR"
...
# ---------------- 1) shared models (public) ----------------
cat > lib/features/tenant/viewings/viewing_models.dart <<'DART'
...
class _ViewingsScreenState extends State<ViewingsScreen> {
  ...
  static const _badgeOrange = Color(0xFFD07A53);

  int _tab = 0; // 0 upcoming, 1 completed, 2 cancelled

  // Demo data (swap to backend later)
  final List<ViewingModel> _upcoming = const [
    ViewingModel(
      id: 'v1',
  ...
  final List<ViewingModel> _completed = const [
    ViewingModel(
      id: 'c1',
  ...
  final List<ViewingModel> _cancelled = const [
    ViewingModel(
      id: 'x1',
  ...
  List<ViewingModel> get _activeList {
    if (_tab == 0) return _upcoming;
    if (_tab == 1) return _completed;
    return _cancelled;
  }

  void _openDetail(ViewingModel v) {
          ...
          body: CustomScrollView(
            ...
            slivers: [
              ...
              if (list.isEmpty)
                SliverFillRemaining(
                  hasScrollBody: false,
                  child: _EmptyState(onBrowse: () => Navigator.of(context).maybePop()),
                )
              else
                SliverPadding(
...
class _ViewingTicketCard extends StatelessWidget {
  ...
  Widget _statusPill(ViewingStatus status) {
    Color bg;
    String text;
        ...
        children: [
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              ...
              child: const Text('Browse listings', style: TextStyle(fontWeight: FontWeight.w900)),
            ),
          ),
        ],
      ),
...
class ViewingDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final canCancel =
        viewing.status == ViewingStatus.confirmed || viewing.status == ViewingStatus.requested;
        ...
        child: Scaffold(
          ...
          body: CustomScrollView(
            ...
            slivers: [
              ...
              // Date/Time card + Add to Calendar
              SliverToBoxAdapter(
                child: Padding(
                  padding: const EdgeInsets.fromLTRB(14, 0, 14, 12),
                  child: _Card(
              ...
              // Location card with mini map + directions
              SliverToBoxAdapter(
                child: Padding(
                  padding: const EdgeInsets.fromLTRB(14, 0, 14, 12),
                  child: _Card(
...
class _Timeline extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final requestedOn =
        status == ViewingStatus.requested || status == ViewingStatus.confirmed || status == ViewingStatus.completed;
    final confirmedOn = status == ViewingStatus.confirmed || status == ViewingStatus.completed;
    final completedOn = status == ViewingStatus.completed;
    ...
    return Column(
      children: [
        Container(
          ...
          child: on ? const Icon(Icons.check, size: 12, color: Colors.white) : const SizedBox.shrink(),
        ),
        const SizedBox(height: 6),
        Text(
          label,
...
echo
echo "✅ Fixed: ClipRRect conflict removed + shared models added + viewings compile clean."
