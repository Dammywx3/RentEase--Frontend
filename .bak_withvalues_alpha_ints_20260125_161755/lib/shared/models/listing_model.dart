  ...
  factory ListingModel.fromMap(Map<String, dynamic> map) {
    ...
    return ListingModel(
      ...
      price: (map['price'] is num)
          ? map['price'] as num
          : num.tryParse((map['price'] ?? '0').toString()) ?? 0,
      currency: (map['currency'] ?? 'NGN').toString(),
      location: (map['location'] ?? map['address'] ?? '').toString(),
      status: (map['status'] ?? 'draft').toString(),
      beds: map['beds'] is int
          ? map['beds'] as int
          : int.tryParse((map['beds'] ?? '').toString()),
      baths: map['baths'] is int
          ? map['baths'] as int
          : int.tryParse((map['baths'] ?? '').toString()),
      type: map['type']?.toString(),
      mediaUrls: urls,
      propertyStatus:
          map['property_status']?.toString() ??
