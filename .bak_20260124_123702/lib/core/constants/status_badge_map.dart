...
class StatusBadgeMap {
  static BadgeSpec forStatus(StatusDomain domain, String raw) {
    ...
    switch (domain) {
      case StatusDomain.verified:
        return _verified[s] ?? const BadgeSpec(label: 'Unknown', tone: BadgeTone.neutral);
      case StatusDomain.listing:
        return _listing[s] ?? const BadgeSpec(label: 'Unknown', tone: BadgeTone.neutral);
      case StatusDomain.maintenance:
        return _maintenance[s] ?? const BadgeSpec(label: 'Unknown', tone: BadgeTone.neutral);
      case StatusDomain.viewing:
        return _viewing[s] ?? const BadgeSpec(label: 'Unknown', tone: BadgeTone.neutral);
      case StatusDomain.property:
        return _property[s] ?? const BadgeSpec(label: 'Unknown', tone: BadgeTone.neutral);
      case StatusDomain.payment:
        return _payment[s] ?? const BadgeSpec(label: 'Unknown', tone: BadgeTone.neutral);
      case StatusDomain.payout:
        return _payout[s] ?? const BadgeSpec(label: 'Unknown', tone: BadgeTone.neutral);
      case StatusDomain.purchasePayment:
        return _purchasePayment[s] ?? const BadgeSpec(label: 'Unknown', tone: BadgeTone.neutral);
  ...
  static const Map<String, BadgeSpec> _listing = {
    ...
    'pending_owner_approval': BadgeSpec(label: 'Pending Approval', tone: BadgeTone.warning),
  ...
  static BadgeSpec _purchaseStatus(String s) {
    ...
    if (s == 'cancelled') return const BadgeSpec(label: 'Cancelled', tone: BadgeTone.neutral);
    if (s == 'refunded') return const BadgeSpec(label: 'Refunded', tone: BadgeTone.neutral);
    if (s == 'closed') return const BadgeSpec(label: 'Closed', tone: BadgeTone.success);

    // default: prettify label
    final label = s.replaceAll('_', ' ');
    final pretty = label.isEmpty ? 'Unknown' : label[0].toUpperCase() + label.substring(1);
