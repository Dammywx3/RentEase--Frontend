...
class _ViewingsScreenState extends State<ViewingsScreen> {
  ...
  final _items = const [
    _ViewingRow(id: 'v1', title: 'Modern 2 Bedroom Apartment', when: 'Jan 25 • 2:00 PM', mode: 'In-person', status: 'approved'),
    _ViewingRow(id: 'v2', title: 'Studio (Close to Road)', when: 'Jan 26 • 11:00 AM', mode: 'Virtual', status: 'pending'),
    _ViewingRow(id: 'v3', title: 'Family Duplex (4 Beds)', when: 'Jan 02 • 3:00 PM', mode: 'In-person', status: 'completed'),
  ];
  ...
  List<_ViewingRow> get _filtered {
    ...
    switch (_tab) {
      ...
      default:
        return _items.where((x) => x.status == 'approved' || x.status == 'pending').toList();
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return AppScaffold(scroll: false,
      ...
      topBar: const AppTopBar(title: 'My Viewings', subtitle: 'Manage your schedule'),
      child: Column(
        children: [
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                _TabChip(label: 'Upcoming', active: _tab == 'Upcoming', onTap: () => setState(() => _tab = 'Upcoming')),
                      ...
                      return ViewingCard(
                        ...
                        onTap: () {
                          Navigator.of(context).push(
                            MaterialPageRoute(
                              builder: (_) => ViewingDetailScreen(viewingId: x.id, title: x.title, status: x.status),
                            ),
                          );
                        },
                        primaryActionText: x.status == 'pending' ? 'Cancel' : 'View',
                        onPrimaryAction: () {
                          Navigator.of(context).push(
                            MaterialPageRoute(
                              builder: (_) => ViewingDetailScreen(viewingId: x.id, title: x.title, status: x.status),
...
class _ViewingRow {
  const _ViewingRow({required this.id, required this.title, required this.when, required this.mode, required this.status});
  final String id;
  final String title;
  final String when;
  final String mode;
