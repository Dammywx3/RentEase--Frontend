...
class MediaStepState extends State<MediaStep> {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        Text('Media', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.w900)),
        const SizedBox(height: AppSpacing.md),
        Text('Uploaded: $mediaCount item(s)', style: Theme.of(context).textTheme.bodyMedium),
        ...
        PrimaryButton(
          ...
          onPressed: () async {
            ...
            if (added != null) {
              ...
              ToastService.show(context, 'Added $added media file(s)', success: true);
            }
          },
        ),
        const SizedBox(height: AppSpacing.md),
        const Text('MVP: shows UI only. Later: upload endpoint + reorder + delete.'),
      ],
    );
  }
}
