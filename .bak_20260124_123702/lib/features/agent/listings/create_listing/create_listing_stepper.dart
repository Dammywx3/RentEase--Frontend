...
class _CreateListingStepperState extends State<CreateListingStepper> {
  ...
  void _next() {
    if (_index == 0 && !(basicsKey.currentState?.validateAndSave() ?? false)) return;
    if (_index == 1 && !(pricingKey.currentState?.validateAndSave() ?? false)) return;
    if (_index == 2 && !(mediaKey.currentState?.validateAndSave() ?? false)) return;
  ...
  Widget build(BuildContext context) {
    ...
    return AppScaffold(
      topBar: const AppTopBar(title: 'Create Listing', subtitle: 'Basics → Pricing → Media → Publish'),
      child: Column(
        children: [
          ...
          child: Padding(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(
                  color: done ? Theme.of(context).colorScheme.primary : Theme.of(context).dividerColor,
                ),
              ),
              child: Center(
                child: Text(
